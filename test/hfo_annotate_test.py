...
from ez_detect.hfo_annotate import _calculateBlockAmount
...
class hfoAnnotateTest(unittest.TestCase):
    ...
    def test_calculate_block_amount__single_block(self):
        blocks = _calculateBlockAmount(self.srate_2000hz, self.cycle_time_10_min, self.samples_2000hz_10_min)
        self.assertEqual(blocks, 1)

    def test_calculate_block_amount__many_full_blocks(self):
        blocks = _calculateBlockAmount(self.srate_2000hz, self.cycle_time_10_min, self.samples_2000hz_1_hour)
    ...
    def test_calculate_block_amount__last_block_must_last_more_than_a_snds_constraint__take_last_block(self):
        n_samples = self.samples_2000hz_10_min + config.BLOCK_MIN_DUR * self.srate_2000hz
        blocks = _calculateBlockAmount(self.srate_2000hz, self.cycle_time_10_min, n_samples)
        self.assertEqual(blocks, 2)

    def test_calculate_block_amount__last_block_must_last_more_than_a_snds_constraint__drop_last_block(self):
        n_samples = self.samples_2000hz_10_min + config.BLOCK_MIN_DUR * self.srate_2000hz - 1
        blocks = _calculateBlockAmount(self.srate_2000hz, self.cycle_time_10_min, n_samples)
        self.assertEqual(blocks, 1)
