...
function dsp_monopolar_output = ez_detect_dsp_monopolar(ez_tall_m, ez_tall_bp, metadata, paths);
    ...
    if error_flag == 0 
        ...
        z_lost_peaks = detectOverstrippedRecordings(hfo, ic1);
            ...
            while eeg_index < (numel(hfo(1,:))-(59.9*2000))
                ...
                for channel=1:numel(hfo(:,1))
                    ...
                    for j=start_index:step_search:numel(zscore_amp);
                        ...
                        if ((flagged==1) && (zscore_amp(j)<-0.2))% Optimization factor #2
...
function z_lost_peaks = detectOverstrippedRecordings(hfo, ic1)
    fprintf('Compensating for overstripping ripple ica \r');
    hfo_rows = numel(hfo(:,1));
    lost_peaks = [];
    delta_amp_peak = zeros(hfo_rows,1);
...
function [ai, hfo_ai] = calculateArtifactIndex(hfo, ic1)    
    fprintf('Calculating artifact index \r')
    ...
    ai=max(ai);
    hfo_ai=ai; %I guess this can be unified with ai in the rest of the code later
    fprintf('Done calculating artifact index \r')
end

function nan_ica = compensateOverstrippingHFO_IC1(hfo, z_lost_peaks)
...
function [thresh_z, thresh_zoff] = skewnessCorrection(zscore_amp_ripple, num_of_m_channels, ...
                                                      thresh_z_values, thresh_zoff_values)
...
function [hfo_times, hfo_values] = convRippleClips_timeToIndices(num_of_data_rows, ripple_clip_t, ic1, total_ripple)
