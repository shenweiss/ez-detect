...
if ~isdeployed
    addpath /home/tpastore/hfo_engine_1
end
...
SC  =  ['/home/tpastore/hfo_engine_1/binica.sc'];           
if ~exist('SC')
...
CUDAICABINARY='/home/tpastore/hfo_engine_1/cudaica'
if exist(CUDAICABINARY) ~= 2
  fprintf('cudaica(): ica binary ''%s'' is not in your Matlab path, check\n', CUDAICABINARY);
  return
else
	CUDAICABINARYdir = '/home/tpastore/hfo_engine_1'
...
scriptfile =  ['/home/tpastore/hfo_engine_1' '/' scriptfile];
...
if ~ischar(data) % data variable given
  ...
  nchans = size(data,1);
  nframes = size(data,2);
  tmpdata = ['/home/tpastore/hfo_engine_1' '/' 'cudaica' tmpint '.fdt'];
...
for x=1:length(flags)
  ...
  elseif strcmp(flags{x},'WeightsOutFile')
     ...
     weightsfile =  ['/home/tpastore/hfo_engine_1' '/' weightsfile];
     args{x} = weightsfile;
  ...
  elseif strcmp(flags{x},'SphereFile')
     ...
     spherefile =  ['/home/tpastore/hfo_engine_1' '/' spherefile];
     args{x} = spherefile;
...
if exist('wtsin') % specify WeightsInfile from 'weightsin' flag, arg
     if exist('wtsin') == 1 % variable
       winfn = ['/home/tpastore/hfo_engine_1/' tmpint '.inwts'];
       if strcmpi(computer, 'MAC')
     ...
     elseif exist(wtsin) == 2 % file
       weightsinfile = wtsin;
       weightsinfile =  ['/home/tpastore/hfo_engine_1' '/' weightsinfile];
     else
...
fprintf('\ncudaica(): ica script file %s data %s pwd %s.\n',...
                                   scriptfile, datafile, '/home/tpastore/hfo_engine_1/');
%
% %%%%%%%%%%%%%%%%%%%%%% run binary ica %%%%%%%%%%%%%%%%%%%%%%%%%
%
