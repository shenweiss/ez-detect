function [results] = zftRipAlpha_0(varargin)
% zftRipAlpha_0.m by Zachary J. Waldman
% 
% Fast Ripple Detector using time frequency analysis combined with 
% topographical techniques. Includes upper frequency reflex test, as well 
% as a seperate lower frequency reflex test.
% Input:
...
% tLow = 0.2;
% tHigh = 0.4;
fmin = 150;                 % Min Frequency for Analysis
fmax = 600;                 % Max Frequency for Analysis
...
fLowR = 165; 
    ...
    if isempty(C(ind)) || thresh < 1 || size(find(C(:) > thresh),1) > size(C,1)
        ...
        for ee = 1:size(ind,1)
            ...
            elseif isequal([C(ind(ee)+1,2) C(ind(ee)+C(ind(ee),2),2)], [min(vdat0) min(vdat0)]);        % (#1)
        ...
        elseif length(clp_grps) >= 1
            ...
            elseif  size(cGrp,1) > 1    % if there are multiple groups within time constraints.
                if sum([dtEv{[cGrp{:,2}],:}]' > 0) > 0 % Make sure there is at least 1 group whose t_wm is positive
...
%     % REFLEX TESTING AT 80HZ
    if det ~= 0
        if min(cgrpF{1,1}{end,1}(:,2)) <= fLow+1.5
   ...
   if ~isempty(olc)
       [results(2,:)] = zRipAlpha_reflex_0(eeg);
   end
