function mat2trc32_msel_bp2k_a(patient_id,file_block)
load('/home/tomas-pastore/ez_detect/putou/bp_temp_trc/eeg_2k_a.mat')
TRC.data=[];
...
trcfile='/home/tomas-pastore/ez_detect/putou/bp_temp_trc/EEG_1_a.TRC'
fid=fopen(trcfile,'r+');
if fid==-1
...
name=   'xxxxx               '
eeg_id=[patient_id '_bp1_' file_block];
...
for i=2:(test_size+1)
  chanstr=eeg.chanlist{i-1}(1:end);
  chanstr=['M_' chanstr];
  c=numel(chanstr)
  positive_input{i}(1:c)=chanstr;
end;
...
fclose(fid);
trcfilename=[patient_id '_bp1_' file_block '.TRC'];
...
position=[];
 for i=1:Num_Chan
     ...
     TRC.data((Num_Chan-(i-1)),:)=TRC.data((Num_Chan-(i-1)),:)-mean(TRC.data((Num_Chan-(i-1)),:))
