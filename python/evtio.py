...
def _appendEventsOfKind(aKindOfEvent, events, rec_start_time, eventsElem, evt_def_guid, 
    ...
    if len(channels) > 0:
        ...
        for i in range(len(channels)):
            ...
            eTree.SubElement(evt, "DerivationInvID").text = str(channel_id) #Channel number in original montage, starting by 1. 
            eTree.SubElement(evt, "DerivationNotInvID").text = "0" #0 for referential. A channel number if bipolar.
...
def write_evt(output_filename, trc_path):
