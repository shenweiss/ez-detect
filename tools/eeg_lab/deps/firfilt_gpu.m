...
function [EEG, delay] = firfilt(EEG, b, nFrames)
...
for iDc = 1:(length(dcArray) - 1)
        ...
        gpu_data=double([EEG.data(:, ones(1, groupDelay) * dcArray(iDc)) ...
                                  EEG.data(:, dcArray(iDc):(dcArray(iDc) + ziDataDur - 1))]);
        % Pad beginning of data with DC constant and get initial conditions
        
        [temp, zi] = filter(b, 1, gpu_data, [], 2);

        blockArray = [(dcArray(iDc) + groupDelay):nFrames:(dcArray(iDc + 1) - 1) dcArray(iDc + 1)];
        for iBlock = 1:(length(blockArray) - 1)
            gpu_data=double(EEG.data(:, blockArray(iBlock):(blockArray(iBlock + 1) - 1)));
            % Filter the data
            [temp_block, zi] = ...
                filter(b, 1, gpu_data, zi, 2);
            EEG.data(:, (blockArray(iBlock) - groupDelay):(blockArray(iBlock + 1) - groupDelay - 1))=temp_block;
        ...
        temp_pad=double(EEG.data(:, ones(1, groupDelay) * (dcArray(iDc + 1) - 1)));
        temp_po = filter(b, 1, temp_pad, zi, 2);

        EEG.data(:, (dcArray(iDc + 1) - ziDataDur):(dcArray(iDc + 1) - 1)) = ...
            temp_po(:, (end - ziDataDur + 1):end);
